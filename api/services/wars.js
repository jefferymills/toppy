...
class WarService {
  ...
  getUserList(userId, warId) {
    return sequelize.query(
      `SELECT *,
      COALESCE(w, 0) as wins,
      COALESCE(l, 0) as losses
      FROM Contestants
      LEFT JOIN ( SELECT winnerId, count(*) as w FROM Battles WHERE userId = ${userId} GROUP BY winnerId) as b ON Contestants.id = b.winnerId
      LEFT JOIN ( SELECT loserId, count(*) as l FROM Battles WHERE userId = ${userId} GROUP BY loserId) as b2 ON Contestants.id = b2.loserId
      WHERE warId = ${warId} ORDER BY wins DESC;`,
      { type: sequelize.QueryTypes.SELECT },
    );
  }
}
